...
**Background jobs launched 2026-06-12 night:** `scripts/sweep-country.js` (AniList countryOfOrigin verification — DELETES non-JP strays like Kan Kluai/Release That Witch, flags adult, JSONL report at `db/country-sweep-report.jsonl`; ~15 min) and `scripts/backfill-source.js` (signal #7 `anime.source` via Jikan, ~4.5h, resumable). NOTE: a Jikan GENRE crawl was attempted and abandoned — MAL genre ids collide with our seeded genre table ids (corrupted rows 1–8 were purged and reseeded); genres come from `seed-genres-from-tags.js` only. Signal #7 is now WIRED (2026-06-12, null-safe — see "Signals #4–#8" block below); the signal strengthens automatically as rows fill, no re-wiring needed when it completes. The backfill died twice (at ~591 and ~1,337 rows) — both times because it was launched as a child of the Claude session, which kills its background tasks on exit, NOT a crash; it now runs detached (`nohup caffeinate -di … &`, logging to `db/source-backfill.log`). Root-caused the constant 429s while at it: Jikan's binding limit for long crawls is **60 req/min**, not 3 req/sec — `src/jikan.js` now paces at 1100ms (~55/min). The catalog source filter and `npm run enrich:art` re-run are DONE (2026-06-12); the source strip grows on its own as rows fill.

**JSON API + taste-vector recompute are LIVE (2026-06-14).** `src/routes/api.js` (the JSON third of the locked routes split) exposes the user's list: `GET /api/library` (optional `?status=` filter), `GET /api/library/:animeId` (one entry, for the anime page's "in your list" state), `PUT /api/library/:animeId` (upsert — `status` required + optional `score` 0-100, `episodes_watched`, `rewatched_count`, `is_favorite`, `notes`), `DELETE /api/library/:animeId`. JSON-native auth gate (`requireUser` → 401, NOT the page redirect); CSRF via the existing global middleware (mutations send `X-CSRF-Token`); `express.json()` added in `server.js`. The domain layer is `src/library.js` (mirrors how auth.js sits behind routes/auth.js — NOT in db.js, which stays the connection): CRUD on `user_anime`, status enum as the single source of truth, auto-stamped `started_at`/`finished_at`, FK miss → clean 404. **Taste vector recompute** runs after every mutation so `user_vectors.synopsis_taste_vec` can't drift from the list: weighted sum of each watched anime's `synopsis_vec` (`tasteWeight` — explicit score maps 0-100→-1..+1 around neutral 50; else status-inferred: COMPLETED/REWATCHING +0.6, WATCHING/PAUSED +0.3, DROPPED -0.6 the negative-signal feature, PLANNING 0; +1.0 favorite; +0.3/rewatch capped at 3), L2-normalized; near-zero or empty-list → NULL. v1 computes ONLY the synopsis facet — tag is TF-IDF not a stored 384-d embedding, character/review vectors are v2; those columns stay NULL, nothing faked. Verified end-to-end (16 assertions, ad-hoc): single favorite → taste cos=1.0 to that anime; adding a DROPPED show pulls the vector away (fav cos 1.0→0.929, dropped show lands at -0.178); CRUD/validation/FK/timestamps/clear-on-empty all pass. NOT yet built (deliberately their own later slices): the onboarding pick-5 quiz (the engine half is now trivial — quiz just calls `upsertEntry` per pick then stamps `users.onboarding_completed_at`), and the "link AniList while logged in" settings button. Next per build order: **views** (Library page, anime-page track controls, onboarding) — all consume this API + `src/library.js`.

**`recommendFromUser` (personalized "for you") is LIVE (2026-06-14).** The consumer the taste vector was built for, in `src/recommender.js` alongside `recommendFromAnime` (parallel name, parallel shape). `recommendFromUser(userId, opts)`: reads `user_vectors.synopsis_taste_vec`, finds nearest catalog anime via sqlite-vec `vec_distance_cosine` (the taste BLOB bound straight in as a 384-d float32, adult-filtered), and ranks by cosine similarity (`score = 1 - dist`, in [-1,1]). Three things layered on top: (1) **excludes everything already on the user's list** in ANY status — a rec for a show you're already tracking is noise; (2) **franchise filter default-on** — reuses `franchiseExcluder` over the *positively-weighted* seeds (so loving Death Note doesn't recommend its sequels), one depth-3 relation walk per positive seed — trivial for a pick-5, the line to revisit only if libraries hit the thousands; (3) **`because` explainability** — each rec is anchored to the positive seed it's most cosine-closest to (`{ id, title, similarity }`), the recommender-spec "because you loved X" feature the Library/onboarding views will render (null when the vector is driven purely by negative signals). Title-dedupes catalog twins like `mergeSignals`. Returns `[]` when there's no usable taste direction yet (empty list, or positives/negatives cancelled to ~zero → `recomputeTasteVector` stored NULL) — the caller treats `[]` as "onboard / add shows first," not an error. **v1 ranks on the SYNOPSIS facet alone** (the only facet `recomputeTasteVector` populates); tag/character/review taste blending is a documented v2 extension point — nothing faked. `tasteWeight` is **lazy-`require`d** from `library.js` inside the function because `library.js` requires `recommender.js` at load — a top-level back-require would hand over a half-initialized module. Smoke test (`npm run test:recommender`) gains a non-mutating Phase 3 that runs against any real user already carrying a taste vector and skips cleanly when none exists. Verified end-to-end against the live catalog (temp user in a rolled-back transaction, nothing persisted): Death Note + FMA:B as favorites → Baccano! anchored to FMA:B (same studio Bones), Oshi no Ko / JoJo: Stone Ocean / Texhnolyze anchored to Death Note, zero listed anime, zero franchise leakage, all `signals: 'taste'`. **`GET /api/recommendations` is LIVE (2026-06-14)** — the JSON surface for these, in `src/routes/api.js` alongside `/api/similar`: behind the `/api` `requireUser` 401 gate, optional `?limit=` (1-48, default 24), calls `recommendFromUser` then hydrates each rec into poster fields (`cover_large` via `largeCover`, dropping cover-less rows, over-fetching `limit+8` so the grid doesn't starve) while preserving the recommender's order and carrying `score` + `because` through for the view to explain each pick; no taste direction yet → `{ recommendations: [] }`. The Library "for you" rail and the onboarding-result screen consume this. Verified by replaying the handler body in a rolled-back transaction (limit honored + clamped, covers hydrate, order preserved, `because` present, empty-list → `[]`).

**Auth is LIVE — email/password + AniList OAuth (2026-06-14).** Email/password (signup, login, logout, sessions, CSRF, bcrypt) shipped in `785ffe4`; `src/auth.js` + `src/routes/auth.js`, sessions in their own `db/sessions.sqlite`. **AniList OAuth now also live** (standard OAuth2 Authorization Code Grant, verified end-to-end against a real AniList account): `GET /auth/anilist` mints a `state` CSRF token + redirects to consent; `GET /auth/anilist/callback` verifies state, exchanges the code (`src/anilist.js` `exchangeCode` → `anilist.co/api/v2/oauth/token`), fetches `Viewer{id,name}` (`fetchViewer` — both bypass the 2200ms catalog rate-limiter, they're one-off user logins not crawl traffic), then `auth.findOrCreateAnilistUser` handles three cases: returning user (recognized by `external_accounts.provider_user_id`, token refreshed), logged-in user LINKING (keyed on live session — AniList exposes NO email so we can't match on it; guarded against hijacking an account already linked elsewhere), and fresh auto-create. **Schema change:** `users.email` is now nullable (`TEXT UNIQUE`, not `NOT NULL`) — AniList-only accounts store `email = NULL` + `password_hash = NULL`; SQLite counts NULLs as distinct so UNIQUE holds. Username seeded from the AniList name, suffixed `-2/-3…` on collision. AniList tokens are JWTs valid ~1 year, stored in `external_accounts` with `expires_at`. The login/signup views carry an equal-weight (outlined, never louder than email — project rule) "continue with AniList" button. OAuth is OPTIONAL: missing `ANILIST_CLIENT_ID/SECRET/REDIRECT` → routes show a clean "not configured" message, no crash (unlike `SESSION_SECRET` which still crashes on boot). DEFERRED to the API/profile milestone: the "link AniList while already logged in" *button* (the callback logic already handles it; just no settings-page entry point yet). Next: **API routes** (`src/routes/api.js`) — JSON surface for list/track/rate, which also unlocks the Library tab.

**Onboarding (cold-start quiz) is LIVE (2026-06-14).** A freshly signed-up user (email or AniList) is gated into picking the anime they've loved, which seeds the taste vector the personal recommender reads. Pieces:
- **Gate:** `requireOnboarding` in `src/auth.js`, mounted globally in `server.js` right after `attachUser`/`csrf`. A logged-in user whose `users.onboarding_completed_at IS NULL` is redirected to `/onboarding` on any GET navigation. Carve-outs that let the flow finish: paths under `/api`, exactly `/onboarding`, and `/logout`; POSTs are never intercepted. `req.user` already carries the column (`attachUser` does `SELECT *`). Stores `returnTo` so finishing bounces the user back where they were headed (verified: gated `/catalog` → finish → lands on `/catalog`).
- **Routes:** `GET/POST /onboarding` in `src/routes/pages.js`. GET builds the starting grid via `onboardingSeeds()` — top 200 by `COALESCE(popularity,0)` within the shared `QUALITY` filter, greedily **franchise-deduped to 54** using the recommender's `franchiseIds()` (depth-3 relations walk, signal #12) plus `titlesAreKin()` as the backstop for cross-id-less twins (one Naruto, not three). POST normalizes the `pick` checkboxes (urlencoded → string | array | undefined) to unique positive int ids, calls `library.upsertEntry(userId, id, {status:'COMPLETED', is_favorite:true})` per pick (each recomputes the taste vector — fine at quiz scale), then `auth.completeOnboarding(userId)` stamps the timestamp. **Zero picks = skip:** same handler, no pick rows written, vector stays empty → recommender's popularity fallback; completion is still stamped so the gate releases. (No separate skip URL — one URL per action.)
- **Picker UX (locked with user):** no minimum pick count — copy *encourages* more ("the more you choose, the stronger the thread") but never blocks; **no-JS checkbox grid** is the base (peer-checkbox CSS draws the selected ring/badge, plain POST submits — works with JS off), with **Spotify-style progressive expansion** layered on: picking a poster fetches `GET /api/similar/:animeId?exclude=<csv shown ids>` (in `src/routes/api.js`, reusing `recommendFromAnime` → the same engine as the anime-page red thread) and **weaves the results into the SAME single grid** (`#seed-grid`), not a separate captioned section — so the grid becomes one never-ending list (user direction 2026-06-14: "transition fully by adding it in the ongoing list … never-ending"). View `views/onboarding.ejs`, script `public/js/onboarding.js` (graceful — no-ops without `fetch`).
- **Expansion dedup (fixed 2026-06-14):** the first cut only deduped by exact id, so other seasons of already-shown titles leaked back in (user caught "many seasons of the previously mentioned anime"). `/api/similar` now takes the `exclude` list (every id on the page) and drops any candidate sharing a franchise with anything shown — the same two-test dedup as the seed grid: `franchiseIds()` relations walk (catches differently-named siblings like Gensoumaden Saiyuuki vs Saiyuuki Reload, which `recommendFromAnime`'s own result set can contain several of) **plus** `titlesAreKin()` for the relation-less twins. `claimed` accumulates franchises within the batch too, so siblings can't pile up in one response. Cost: <=30 depth-3 walks per per-pick fetch — fine. Verified: Naruto's similar collapses the three Saiyuuki entries to one; `exclude` drops named ids end-to-end over HTTP.
- **Refactor:** `largeCover()` extracted from `pages.js` into `src/covers.js` so the api route shares it (DRY).
- Verified end-to-end against a live server: signup → bounced to `/onboarding` (and `/catalog` bounced too) → 54 deduped posters → `/api/similar` returns 8 → POST 2 picks writes COMPLETED+favorite rows, builds a non-NULL `synopsis_taste_vec`, stamps `onboarding_completed_at`, gate releases; the zero-pick skip path stamps completion with no rows. Test accounts cleaned up afterwards.
- DEFERRED: the **Library tab** (still an inert `nav-tab-soon` span) — the `/api/library` surface + `recommendFromUser` exist; the page that renders them is the natural next build.

**Popularity backfill + damped ranking (2026-06-12 morning, MILGRAM incident):** user spotted "Milgram Dainishin" (a YouTube music-video project, 9.35 from a tiny devoted fanbase, both synopses empty) topping the home podium — `anime.popularity` was NULL on every row (offline-DB doesn't carry it), so nothing damped small-sample scores; only the blunt `< 9.4` cutoff stood, and 9.3528 slid under it. Fix in three parts: (1) `scripts/backfill-popularity.js` (`npm run backfill:popularity`, in the `refresh` chain after sweep) fills popularity from AniList batched 50/req (~14 min for 18K rows; sweep-country pattern; phase 2 catches MAL-only leftovers via Jikan — run that only when backfill:source isn't running, shared 60/min budget); (2) `scripts/backfill-source.js` now ALSO writes MAL `members` into popularity (guarded `popularity IS NULL` so AniList's scale wins on dual-id rows; scales differ ~5x, fine for damped ranking — see schema comment) — the source crawl was restarted with this (resumes cleanly); (3) **damped score ordering — DONE (2026-06-13).** `src/routes/pages.js` now defines a `DAMPED` SQL fragment — Bayesian/IMDb `(pop*score + m*C)/(pop + m)`, `pop = COALESCE(a.popularity,0)`, `C` = catalog mean score (`AVG(average_score)` over the QUALITY set, queried once at module load ≈ 6.18, with a 6.18 fallback if the query can't run), `m = DAMP_M = 5000` (sits below every legit top title's popularity — all > 100k, so Frieren/FMA:B move < 0.04 — and above the noise band: MILGRAM's 2,143 votes → 9.35 damps to 7.13). It's the ORDER BY on the home top-8 podium, the three score-ranked rails (映画 / 九十年代 / 時間旅行 — the 今季 airing rail stays popularity-sorted), the focus-shelf top title, and `/catalog?sort=score` (final term only; the unfinished/≥9.4 demotions are kept ahead of it). Verified: MILGRAM (id 22027) is absent from the home page and `/catalog?sort=score` p1, and now ranks #2175 in the catalog score sort; Frieren stays #1 on the podium. NOTE: fixing this surfaced and fixed a *separate* pre-existing crash — `views/home.ejs:252` called `a.average_score.toFixed(2)` unguarded, and the 今季 airing rail can carry null-score shows (the old running server predated the data state that exposed it); now wrapped in an `!= null` guard. ALSO: after backfill-source completes, run `npm run backfill:popularity` once more — its phase 2 fills MAL-only leftovers via Jikan (never run phase 2 while backfill-source runs, shared 60/min budget). CAUTION learned: check `ps` for an already-running crawl before launching one — two source crawls briefly ran in parallel (one user-restarted at 5:39) and double-burned the Jikan minute budget.

**BACKFILLS COMPLETE (2026-06-13).** Both crawls (which the SHUTDOWN STATE below left mid-run) ran to completion this session and the damped ranking shipped — the whole popularity/source/damping arc is closed:
- **source backfill: COMPLETE** — `backfill-source: COMPLETE — 28233 anime now carry source material. 60 misses.` 25,806 rows carry a real (non-`Unknown`) source. The ~10.3K still-NULL rows have no mal_id, so they're permanently uncoverable (and `QUALITY` excludes them from ranking anyway). The catalog source filter is fully populated.
- **popularity backfill: COMPLETE** — AniList lane done (18,084 rows), MAL-`members` popularity filled by the source crawl as it went, and a final `npm run backfill:popularity` phase-2 pass mopped up the last MAL-only stragglers (5/53 had a members count; the rest don't on MAL either). Remaining popularity-NULL ≈ 8,994 — all rows with neither a usable anilist_id nor MAL members data, the permanent floor; none rank.
- **damped ranking: LIVE** — see the MILGRAM block above. Verified MILGRAM off every surface, Frieren #1.
- Next, per the locked build order: ~~auth → AniList OAuth~~ **DONE 2026-06-14 (see the Auth block at the top)** → API routes. No backfill work remains.

**SHUTDOWN STATE (2026-06-12 ~06:00 IST — machine was being turned off; both crawls killed mid-run, both checkpoint-resumable, nothing lost) — SUPERSEDED, kept for history:**
- popularity backfill: **13,566 rows filled**, AniList lane was at 13,050/18,084 (~75%, ~4 min of work left). Resume: `nohup caffeinate -di node scripts/backfill-popularity.js >> db/popularity-backfill.log 2>&1 &`
- source backfill: checkpoint past id ~6,600; ~23,000 rows left (~7h). Resume: `nohup caffeinate -di node scripts/backfill-source.js >> db/source-backfill.log 2>&1 &` (it now also fills popularity from MAL members as it goes)
- Resume order on next pickup: (1) start both crawls (different APIs, safe in parallel); (2) ~~when the popularity AniList lane completes, implement the damped ordering~~ **DONE 2026-06-13** — popularity AniList lane finished, damped ordering shipped (see the MILGRAM block above); the redundant popularity phase-2 crawl was killed (source crawl fills popularity from MAL members anyway, and the shared-Jikan-budget rule forbids running both); (3) after the source crawl completes, `npm run backfill:popularity` once more for the phase-2 Jikan leftovers.
...
**Signal #12 (relations / franchise filter) is LIVE (2026-06-12).** Shipped early (was waiting on `user_anime`) after user feedback: Mushishi's rec list was five Mushishi sequels/specials — "nobody wants to know Mushishi is similar to Mushishi." `recommendFromAnime` now defaults `excludeFranchise: true` (anime page AND discover get it; pass `false` for the raw ranking). Mechanism in `src/recommender.js`: `franchiseExcluder()` = depth-3 walk over `relations` (NOT full transitive closure — v1 relations are uncategorized, so crossover specials chain Fate/DBZ/Lupin into ONE ~3,485-anime component, ~9% of the catalog; depth 3 covers real franchises — Mushishi needs 3 hops to reach Suzu no Shizuku — at a worst measured blast radius of ~244 for DBZ) **plus** title-kinship (`titlesAreKin`: exact or word-boundary-prefix match against any walk member's titles), because 22.4K/38.5K rows have ZERO relations — the offline-DB carries some entries twice (per source site) and the cross-id-less twin gets no relatedAnime (measured leaks: a second FMA:B row topped its own twin's list; "Death Note Rewrite 1/2" leaked past exact-match). Accepted over-exclusions, both invisible against a 100-candidate pool: DBZ's walk swallows crossover partner One Piece; "Monster" title-swallows "Monster Musume". `mergeSignals` applies the predicate BEFORE min-max normalization (a franchise near-perfect match would set the ceiling and compress real candidates) and the final ranking dedupes by display title (two Baccano! rows both surfaced). When v2 brings categorized relation types: stop traversing crossover/character edges instead of capping depth. Smoke test (`npm run test:recommender`): 37 assertions incl. Mushishi + Death Note end-to-end; Death Note → Monster is now the #1 rec.
...
- `views/home.ejs` — three-act home page (paper hero → dark content act with podium + rails → paper signals colophon); `views/catalog.ejs` (search/filters/grid/pager, GET-only, no JS), `views/anime.ejs` (dark hero, both synopses w/ attribution, tag chips → catalog, 赤い糸 recommendations w/ signal provenance, relations); **docs are a small multi-page system (2026-06-12, user direction: TLDR first, depth split across pages, simple words, NO hyphens in visible copy)** — `views/how-it-works.ejs` is the index (brief + 要約 TLDR + chapter map), chapters live in `views/docs/{library,fingerprints,signals,ranking}.ejs` with shared `views/partials/docs-header.ejs` + `docs-pager.ejs` chrome, whitelisted in `DOC_CHAPTERS` in `src/routes/pages.js` (unknown slug 404s); signal copy (short `blurb` for grids + long `detail` for the signals chapter) lives in `src/signals.js`; `views/partials/{head,nav,footer,enso,frame,cover-card}.ejs` shared chrome; footer carries the required Wikipedia CC BY-SA + Jikan attributions (proper names keep their hyphens)
...
Same-franchise dominance still present (FMA → all FMA). ~~That's correct for "more like this" mode~~ **REVERSED 2026-06-12 (user feedback: "nobody wants to know Mushishi is similar to Mushishi") — signal #12 now filters the franchise out of ALL rec queries by default.** See the "Signal #12" block under Current state.
...
6. **After all categorical signals: ranking refinements.** Signal #11 (popularity floor + serendipity re-rank) waits for `user_anime` data which doesn't exist yet. ~~Signal #12 (relations filter for discover mode)~~ **DONE 2026-06-12** — shipped early as a default-on franchise filter after user feedback; see the "Signal #12" block under Current state.
...
**`isAdult` alone is INSUFFICIENT — `is_adult` is also set by tag/format rules (2026-06-14).** The import only carries AniList's `isAdult`, so MAL-only and recent OVAs (no `anilist_id`) leaked hentai onto `/catalog` and `/airing` (user caught hentai OVAs on the live site). `scripts/flag-adult.js` (`npm run flag:adult`, in the `refresh` chain, idempotent) sets `is_adult = 1` via four rules: (1) the `hentai` tag — precise, 1,702 titles, no false positives on the kept gore/violence seinen; (2) `erotica`-tagged OVA/SPECIAL/MOVIE that aren't BL-tagged and aren't on a small mainstream keeplist — the porn the `hentai` tag missed (Urotsukidoji, Cream Lemon, the Chinese-language erotica films); (3) explicit BL/yaoi — all `erotica`+BL except a non-explicit keeplist (Junjou Romantica, Sekaiichi Hatsukoi, Yami no Matsuei, Gravitation), a deliberate aggressive call that also sweeps acclaimed-but-explicit BL (Saezuru, Ai no Kusabi); (4) tag-less duplicate twins — the offline-DB stores some titles twice and the cross-id-less twin has no tags, so a clean row sharing an adult row's title AND year (same work — year guard avoids name-collisions like Tezuka's 1968 "Vampire" vs a 2011 hentai) gets flagged too. ~1,896 titles flagged total. KNOWN RESIDUE intentionally left visible: ~26 mainstream TV ecchi (To Love-Ru, Yosuga no Sora) and the BL keeplist; the `erotica`/`nudity`/`sex` tags are NOT blanket signals (they sit on Devilman Crybaby, Berserk). The Chinese-language films are also out-of-scope non-JP and should ideally be DELETED by a country sweep, not just hidden.
...
4. **AniList OAuth credentials.** User must register an app at https://anilist.co/settings/developer and provide `client_id`, `client_secret`, `redirect_uri`. These go in `.env`. (Active — user starting this now.)
   ...
   **Captured feature for the auth milestone — sakura account corner (locked 2026-06-12).**
   User idea, planned and decision-locked, deliberately NOT built before auth exists
   (no fake features): an ink-brush **sakura branch with petals** in the upper-right
   of the home hero (above the 桜 kanji, which is `pointer-events-none`) that on
   hover/focus/tap reveals a **"glass over washi" panel** (backdrop blur + paper tint
   + grain + hairline ink border — first backdrop-filter in the codebase, keep it
   scoped). Logged in: PFP (consider `.enso-mask` brush circle) linking to `/profile`.
   Logged out: sign-in invitation. Must be keyboard/touch accessible, respect reduced
   motion, no-JS fallback = plain link, and never be the only path to the profile
   (the Library nav tab goes live the same milestone). Full spec:
   `~/.claude/plans/let-s-think-about-what-declarative-moler.md`.
