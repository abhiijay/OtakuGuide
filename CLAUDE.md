...
- Resume order on next pickup: (1) start both crawls (different APIs, safe in parallel); (2) when the popularity AniList lane completes, implement the damped ordering per the spec above and verify MILGRAM is off the podium — that doesn't need the source crawl to be done; (3) after the source crawl completes, `npm run backfill:popularity` once more for the phase-2 Jikan leftovers.
